...
public class AltinnMeldingHendelse {
	...
	public void publish(CloudEvent cloudEvent) {
		...
		ProducerRecord<String, CloudEvent> cloudEventProducerRecord = new ProducerRecord<>(
				topicsProperties.altinnMeldingHendelse(),
				cloudEvent.getId().toString(),
				cloudEvent
		);

		try {
			kafkaTemplate.send(cloudEventProducerRecord)
					.whenComplete((_, ex) -> {
						if (ex != null) {
							handleKafkaError(topic, ex);
						} else {
							log.info("cloudEvent med (id={}, resourceinstance={}) skrevet til topic: {}.",
									cloudEvent.getId(), cloudEvent.getResourceinstance(), topic);
						}
					}).get();
